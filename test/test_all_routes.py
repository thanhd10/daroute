...
from test.settings import TEST_RESULT_TARGET_DIR, \
    LOG_TARGET_DIR, TARGET_LOCATION, AREA_TARGET_PATH, SAMPLES_DIRECTORY
...
class TestRouteCandidates(unittest.TestCase):
    ...
    def test_all_routes(self):
        if len(parse_all_directories(SAMPLES_DIRECTORY)) == 0:
        ...
        for directory in parse_all_directories(SAMPLES_DIRECTORY):
            self.logger.info('-' * 80)
            test_route = create_next_test_route(self, directory)
            test_route_sensor(self, self.turns_df, test_route)


def parse_all_directories(directory):
    """
    ...
    for path_name, _, file_names in os.walk(directory):
        for file_name in file_names:
            ...
            if "Route_Sensor" in file_name:
                directories.add(path_name)

    return sorted(list(directories))


def create_next_test_route(unit_test: TestRouteCandidates, output_dir: str):
    if os.path.exists(output_dir + "/Start_Heading.txt"):
        ...
        raise Exception("No initial heading found in directory %s." % output_dir)

    # extract route id
    string_route_id_start = output_dir.find("Route")
    route_id = output_dir[string_route_id_start:] + "/Route_Sensor"

    # extract osm nodes as the ground truth
    with open(output_dir + "/OSM_Nodes.txt", "r") as fh:
        osm_nodes = list(map(int, fh.readlines()[0].split(",")))

    unit_test.logger.info("Route %s with heading %d and osm nodes %s found."
                          % (route_id, initial_heading, osm_nodes))
...
def test_route_sensor(unit_test: TestRouteCandidates,
...
def run_sensor_preprocess(test_route: TestRouteModel) -> Tuple[List[SensorTurnModel],
    ...
    json_file_path = SAMPLES_DIRECTORY + "/" + test_route.test_id + ".json"
