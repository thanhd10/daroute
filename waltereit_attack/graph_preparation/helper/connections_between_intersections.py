...
from definitions import LABEL_COL_NAME, NODE_ID_COL_NAME, LAT_COL_NAME, LNG_COL_NAME, START_ID_COL_NAME, \
    END_ID_COL_NAME, AGGREGATED_DISTANCE_COL_NAME, INTERSECTION
from waltereit_attack.graph_preparation.schema.RoadElements import IntersectionConnectionElement, \
...
class IntersectionConnectionsCreator(object):
    ...
    def __init__(self, ways_df: pd.DataFrame, nodes_df: pd.DataFrame):
    ...
    def create_connections_for_intersection(self, intersection_id: int) -> [IntersectionConnectionElement]:
        ...
        return flatten_list([self.__add_connection_to_intersection(intersection_id,
                                                                   int(connection[0]),
                                                                   float(connection[1]))
                             for connection in
                             current_connections[[END_ID_COL_NAME, AGGREGATED_DISTANCE_COL_NAME]].values])

    def __add_connection_to_intersection(self,
                                         curr_intersection: int,
                                         next_node_id: int,
    ...
    def __create_dead_end(self, curr_intersection: int, dead_end: int, next_node_id: int, next_distance: float) \
            -> Optional[IntersectionConnectionElement]:
...
def create_connections_between_intersections(ways_df: pd.DataFrame, nodes_df: pd.DataFrame) \
        -> [IntersectionConnectionElement]:
    """
    Endpoint to find and create all connections between intersections
    """
    database_creator = IntersectionConnectionsCreator(ways_df, nodes_df)
